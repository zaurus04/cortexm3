 ...
 * 
 *  @(#) $Id: cpu_context.h,v 1.6 2008/07/15 11:49:03 suikan Exp $
 */

/*
 *	タスクコンテキスト操作ルーチン（CORTEX-M3用）
 *
...
activate_context(TCB *tcb)
{
	...
	*--sp = (VW)(tcb->tinib->task) | 1;		/* 最初にタスクの開始アドレスをプッシュ */
	*--sp = (VW)&ext_tsk | 1;				/* 次にext_tsk()の開始アドレスをプッシュ */
	*--sp = (VW)(tcb->tinib->exinf);		/* 最後にexinfパラメータ */
	tcb->tskctxb.sp = sp;
