...
/*
 * NVIC割り込を含めた例外ベクトルのエントリ数。
 * 
 * STM32F103Vの場合、NVICが管理しない16と、管理する43の計59.
 * 
 * !!!!注意！！！！
 * 例外ベクトルは、そのサイズよりも大きな2の冪[BYTE]に整列しなければならない。整列は .ldファイルで
 * 指示する。
 * ( CORTEX-M3 Technical Reference Manual 8.2.2 NIVC Register Description の
 ...
 * 割り込みスタックのサイズ[BYTE]
 * 
 * TOPPERS/JSPの割り込みが共用するスタック。可能なすべてのネストを吸収できる大きさでなければならない。
 */
 ...
 * 
 * 直感に反するが、ターゲット依存部で用意しないなら、SUPPORT_VXGET_TIMを宣言する。
 */
...
/*
 *  シリアルポート数の定義
 *  CORTEX-M3 : TNUM_PORT, TNUM_SIOPの数に関しては要検討
 */
 CM3NI
