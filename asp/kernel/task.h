 ...
 *  の責任を負わない．
 * 
 *  @(#) $Id: task.h 1159 2008-06-27 18:17:41Z ertl-hiro $
 */
 ...
 *  実行できる状態への移行
 *
 *  p_tcbで指定されるタスクの状態を実行できる状態とし，レディキューに
 *  挿入する．実行できる状態になったタスクの優先度が，最高優先順位のタ
 *  スクの優先度よりも高い場合は，最高優先順位のタスクを更新し，ディス
 *  パッチ許可状態であればtrueを返す．そうでない場合はfalseを返す．
 */
